...
describe('DbAuthentication Usecases', () => {
  ...
  test('Should return null if LoadAccountByEmailRepository return false', async () => {
    const { sut, hashComparerStub } = makeSut()
  ...
  test('Should return a token if TokenGenerator throws', async () => {
    const { sut, tokenGeneratorStub } = makeSut()
  ...
  test('Should return a token if TokenGenerator return', async () => {
    const { sut } = makeSut()
